...
class ClassSparsity(IterativeConstraint):
    ...
    def check_constraints(self, selected_edge_tensor, selected_features):
        problematic_classes, total_missing = check_min_sparsity(selected_edge_tensor,
                                                                self.parameter)

        self.total_relevant_classes = self.total_relevant_classes.union(
            problematic_classes)
...
def check_min_sparsity(edge_tensor, min_sparsity, dim=0):
    ...
    total_missing = (
        min_sparsity - weights_per_class[problematic_classes]).sum()
...
class RemoveCandidateCounter:
    ...
    def add_candidate(self, value, index):
        ...
        if len(self.values) < self.goal:
            ...
            # Check that this is the highest value in the dict
            comparator, ind = self.values.peekitem()
            if comparator > value:
...
def add_second_order_min_sparsity(problematic_classes,
                                  selected_edge_tensor,
                                  assignment_matrix_selected,
                                  second_order_min,
                                  total_missing):
    ...
    if int(selected_edge_tensor.sum() / selected_edge_tensor.shape[1]) == second_order_min:
        ...
        too_many_features_indices = np.arange(len(too_many_features_indices))[
            too_many_features_indices]
        too_many_features_matrix = selected_edge_tensor[:,
                                                        too_many_features_indices]
        ...
        for single_class in range(too_many_features_matrix.shape[1]):
            ...
            for index in sorted_vals:
                candidate_counter.add_candidate(
                    relevant_similarity[index], (this_class, nonzero_indices[index]))
    ...
    if answer_matrix.sum() == selected_edge_tensor.sum():
        print(
            "Working without starting solution. AssertionError for (answer_matrix.sum() == selected_edge_tensor.sum()) is commented out.")

    return answer_matrix


def add_sparsity_constraints(total_problematic_classes,
                             total_relevant_features,
                             prev_constraints,
                             existing_edges,
                             min_sparsity,
                             model):
    ...
    for single_class in total_problematic_classes:
        this_constr = model.addConstr(existing_edges[total_relevant_features, single_class,].sum() >= min_sparsity,
