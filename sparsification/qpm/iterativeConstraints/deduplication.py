...
from CleanCodeRelease.sparsification.qpm.iterativeConstraints.IterativeConstraint import IterativeConstraint
...
class DeDuplipication(IterativeConstraint):
    ...
    def add_constraints(self, existing_edges, next_start, edges, features):
        diffs, us, self.prev_constraints, self.prev_dubs = clean_add_uniqueness_constraint(self.model,
                                                                                           self.duplicates,
    ...
    def check_constraints(self, selected_edge_tensor, selected_features):
        self.duplicates, self.relevant_classes = get_duplicates(
...
class CheckDuplicates:
    def __init__(self, selected_edge_tensor):
        ...
        self.nonzero_to_index_dict = {i: np.nonzero(
            self.nonzeros == i) for i in np.unique(self.nonzeros)}
        ...
        if raise_error:
            ...
            while len(remining_duplicates) > 0:
                ...
                if len(possible_remover_classes) == 0:
                    ...
                    print(
                        "No possible remover classes, trying sophisticated deduplication")
                    return sophisticated_deduplication(pre_deduplication, similarity_matrix)
                possible_removed_edges = assigned_edges[:,
                                                        possible_remover_classes]
                # initialise to lowest
                max_sim = torch.min(possible_removed_edges)
                possible_removed_edges[possible_removed_edges == 0] = torch.max(
                    possible_removed_edges) + 1

                for sngl_class in [c1, c2]:
                    relevant_row = similarity_matrix[:, sngl_class] * (
                        ~selected_edge_tensor[:, sngl_class])
                    if no_duplicates:
                        would_be_duplicate = True
                        possible_features = np.arange(relevant_row.shape[0])
                        while would_be_duplicate:
                            best_new, best_indices = torch.max(
                                relevant_row[possible_features], dim=0)
                            this_feature_idx = possible_features[best_indices]
                            possible_features = np.delete(
                                possible_features, best_indices)
                ...
                row, col = np.unravel_index(torch.argmin(
                    possible_removed_edges), possible_removed_edges.shape)
                selected_edge_tensor[row,
                                     possible_remover_classes[col]] = False
                selected_edge_tensor[best_idx, best_class] = True
                added.add(best_class)
                remining_duplicates, this_relevant_classes = get_duplicates(
                    selected_edge_tensor)
            ...
            check_duplicate(selected_edge_tensor,
                            similarity_matrix, raise_error=True)
...
class SwapDuplicateChecker:
    def __init__(self, selected_edge_tensor):
        self.selected_edge_tensor = selected_edge_tensor
        self.distance_matrix = sklearn.metrics.pairwise_distances(
            selected_edge_tensor.T, metric='l1')
        self.distance_matrix[np.eye(
            self.distance_matrix.shape[0], dtype=bool)] = np.inf
    ...
    def would_line_be_duplicate(self, questionable_class_index, questionable_feature_index, remove_from_class_index):
        ...
        relevant_classes = np.nonzero(
            self.distance_matrix[questionable_class_index] == 2)[0]
        for second_idx in relevant_classes:
            if (questionable_line == self.selected_edge_tensor[:, second_idx]).all():
                return True
        return False
...
def sophisticated_deduplication(selected_edge_tensor, similarity_matrix):
    ...
    remining_duplicates, relevant_classes = get_duplicates(
        selected_edge_tensor)
    print("Remaining Duplicates Before Sophisticated Deduplication ",
          len(remining_duplicates))
    ...
    while len(remining_duplicates) > 0:
        ...
        if idx >= len(remining_duplicates):
            print("Sophisticated Deduplication failed")
            return selected_edge_tensor
        ...
        for sngl_class in [c1, c2]:
            this_class_tensor = selected_edge_tensor[:,
                                                     sngl_class] * similarity_matrix[:, sngl_class]
            relevant_row = similarity_matrix[:, sngl_class] * \
                (~selected_edge_tensor[:, sngl_class])
            ...
            for remove_feature in assigned_features:
                would_be_duplicate = True
                assignable_features = torch.nonzero(
                    ~selected_edge_tensor[:, sngl_class]).squeeze()

                while would_be_duplicate:
                    best_new, best_indices = torch.max(
                        relevant_row[assignable_features], dim=0)
                    this_feature_idx = assignable_features[best_indices]
                    assignable_features = np.delete(
                        assignable_features, best_indices)
                    would_be_duplicate = noDuplicate_Checker.would_line_be_duplicate(sngl_class, this_feature_idx,
                                                                                     remove_feature)
                    if not would_be_duplicate:
                        total_cost = this_class_tensor[remove_feature] - \
                            relevant_row[this_feature_idx]
        ...
        print("Removed Duplicate ", (c1, c2), "by replacing", (to_remove, best_class),
              similarity_matrix[to_remove,
                                best_class], "with", (best_idx, best_class),
              similarity_matrix[best_idx, best_class], "Remaining ", len(
                  remining_duplicates), "duplicates ",
              remining_duplicates)
...
def clean_add_uniqueness_constraint(model,
                                    duplicates,
                                    previous_constraints,
                                    total_relevant,
                                    prev_dubs,
                                    existing_edges,
                                    existing_edge_tensor):
    diffs = []
    us = []
    ...
    # prev_dubs needs to be a dict with (c1,c2): features previsouly duplicated
    duplicates = set(duplicates).union(set(prev_dubs))
    ...
    for class_idx, second_class in duplicates:
        ...
        diff = model.addMVar(len(feature_indices),
                             lb=-1,
                             vtype=GRB.INTEGER,
                             name="diff")

        u = model.addVar(name="u", vtype=GRB.INTEGER)

        constraints.append(model.addConstr((diff == existing_edges[feature_indices, class_idx]
                                            - existing_edges[feature_indices, second_class]),
                                           name=f"Aux_{class_idx}_{second_class}"))

        constraints.append(model.addConstr(u == gp.norm(diff, 1),
                                           name=f"Norm_Deduplication_{class_idx}_{second_class}"))

        constraints.append(model.addConstr(u >= 1,
                                           name=f"Deduplication_{class_idx}_{second_class}"))
