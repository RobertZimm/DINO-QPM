...
from CleanCodeRelease.scripts.Hierarchy.HierarchyClass import create_pairwise_diff
from CleanCodeRelease.scripts.datasetProblems.createCUB200classMapping import load_cub_class_mapping
...
def get_top_n_similar_pairs_cub(per_class_avg):
    ...
    class_sim_gt, class_sim_cbm = CUB200Class.get_class_sim_from_root(str(base_folder))
    ...
    pairs_to_enforce = get_list_from_pairwise_diff(class_sim_gt_for_this, n_pairs_to_create)
    pairs_to_enforce_names = [(cub_mapping[i], cub_mapping[j]) for i, j in pairs_to_enforce]
