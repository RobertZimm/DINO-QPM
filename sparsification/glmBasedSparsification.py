...
from CleanCodeRelease.sparsification.FeatureSelection import FeatureSelectionFitting
from CleanCodeRelease.sparsification import data_helpers
from CleanCodeRelease.sparsification.utils import get_default_args, compute_features_and_metadata, select_in_loader, get_feature_loaders
...
def get_glm_selection(feature_loaders, metadata,  args, num_classes, device, n_features_to_select, folder):
    ...
    selected_features = torch.tensor(
        [i for i in range(num_features) if i not in to_drop])
    return selected_features


def compute_feature_selection_and_assignment(model, train_loader, test_loader, log_folder, num_classes, seed,  select_features=50, config=None):
    feature_loaders, metadata, device, args = get_feature_loaders(
        seed, log_folder, train_loader, test_loader, model, num_classes, config=config)

    if os.path.exists(log_folder / f"SlDD_Selection_{select_features}.pt"):
        feature_selection = torch.load(
            log_folder / f"SlDD_Selection_{select_features}.pt", weights_only=True)
    else:
        used_features = model.linear.weight.shape[1]
        if used_features != select_features:
            # overwrite with None to prevent saving
        ...
        torch.save(feature_selection, log_folder /
                   f"SlDD_Selection_{select_features}.pt")
    ...
    sparse_matrices, biases = fit_glm(
        log_folder, mean_to_pass_in, std_to_pass_in, feature_loaders, num_classes, select_features)
...
def load_glm(result_dir):
    ...
    params_dict = {i: torch.load(os.path.join(result_dir, f"params{i}.pth"),
                                 map_location=torch.device('cpu'), weights_only=True) for i in range(Nlambda)}

    regularization_strengths = [params_dict[i]
                                ['lam'].item() for i in range(Nlambda)]
