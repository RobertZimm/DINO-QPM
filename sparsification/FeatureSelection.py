...
from CleanCodeRelease.sparsification.utils import safe_zip
...
class FeatureSelectionFitting:
        ...
        with ch.no_grad():
            ...
            if n_classes is None:
                if family == 'multinomial':
                    n_classes = max(tensors[1].max().item()
                                    for tensors in loader) + 1
            ...
            for t in range(nepochs):
                ...
                for n_batch, batch in enumerate(loader):
                    ...
                    if family == 'multinomial':
                        if w is None:
                            loss = F.cross_entropy(
                                out, y.to(weight.device), reduction='mean')
                        else:
                            loss = F.cross_entropy(
                                out, y.to(weight.device), reduction='none')
                            loss = (loss * w).mean()
                        ...
                        if w is None:
                            loss = 0.5 * \
                                F.mse_loss(out, y.to(weight.device),
                                           reduction='mean')
                        else:
                            loss = 0.5 * \
                                F.mse_loss(out, y.to(weight.device),
                    ...
                    w_grad_prev = (a_prev.unsqueeze(
                        2) * X.unsqueeze(1)).mean(0)
            ...
            logger(
                f"did not converge at {nepochs} iterations (criteria {criteria})")
            return {
                "a_table": a_table.cpu(),
                "w_grad_avg": w_grad_avg.cpu(),
                "b_grad_avg": b_grad_avg.cpu()
    ...
    def glm_saga(self, linear, loader, max_lr, nepochs, alpha, dropout, tries,
        ...
        if encoder is not None:
            warnings.warn(
                "encoder argument is deprecated; please use preprocess instead", DeprecationWarning)
            preprocess = encoder
        ...
        lrs = ch.logspace(math.log10(max_lr), math.log10(
            max_lr / lr_decay_factor), k)
        found = False
        if do_zero:
        ...
        if checkpoint is not None:
            ...
            file_handler = logging.FileHandler(
                filename=os.path.join(checkpoint, 'output.log'))
        ...
        while self.selected_features.sum() < self.nKeep:  # TODO checkout this change, one iteration per feature
            ...
            for i, (lam, lr) in enumerate(zip(lams, lrs)):
                ...
                # TODO checkout this change, canceling if new feature is used
                if self.check_new_feature(linear.weight):
                    if checkpoint is not None:
                        ch.save(params, os.path.join(
                            checkpoint, f"params{n_feature_to_keep}.pth"))
                    break
        if found:
            return {
                'path': path,
