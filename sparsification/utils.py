...
from CleanCodeRelease.sparsification import feature_helpers
...
def compute_features_and_metadata(args,
                                  train_loader,
                                  test_loader,
                                  model,
                                  out_dir_feats,
    ...
    # Compute Features for not augmented train and test set if arch is not dinov2
    ...
    for mode, loader in zip(['train', 'test'],
        ...
        feature_ds, feature_loader = feature_helpers.compute_features(loader,
                                                                      ...
                                                                      n_epoch=1,
                                                                      balance=False,
                                                                      config=config)  # args.balance if mode == 'test' else False)
...
def save_feat_loaders(seed,
                      ...
                      num_classes,
                      config,
                      output_features_folder: str = "features"):
    feature_loaders, metadata, _, _ = get_feature_loaders(seed=seed,
                                                          ...
                                                          num_classes=num_classes,
                                                          config=config,
                                                          output_features_folder=output_features_folder)


def get_feature_loaders(seed,
                        log_folder,
                        train_loader,
                        test_loader,
                        model,
    ...
    feature_folder = Path(log_folder) / output_features_folder
    feature_loaders, metadata = compute_features_and_metadata(args=args,
                                                              train_loader=train_loader,
                                                              test_loader=test_loader,
                                                              model=model,
                                                              out_dir_feats=feature_folder,
                                                              num_classes=num_classes,
                                                              config=config)
...
def get_default_args():
    ...
    parser.add_argument('--cache', action='store_true',
                        help='cache deep features')
    parser.add_argument('--balance', action='store_true',
                        help='balance classes for evaluation')
    ...
    parser.add_argument("--multi-seed", action='store_true', default=False,
                        help="sepcifies whether multiple seeds are used in parallel to the current run")
...
def select_in_loader(feature_loaders,
                     feature_selection):
    for dataset in feature_loaders[
            "train"].dataset.dataset.dataset.datasets:  # Val is indexed via the same dataset as train
