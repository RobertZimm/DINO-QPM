...
def compute_sldd_feature_selection_and_assignment(model, train_loader, test_loader, log_folder, num_classes, seed,
                                                  per_class=5, select_features=50, config=None):
...
def get_sparsified_weights_for_factor(sparse_layer, biases, keep_per_class, drop_rate=0.5):
    nonzero_entries = [torch.sum(torch.count_nonzero(
        sparse_layer[i])) for i in range(len(sparse_layer))]
    mean_sparsity = np.array(
        [nonzero_entries[i] / sparse_layer[i].shape[0] for i in range(len(sparse_layer))])
