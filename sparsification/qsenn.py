...
from CleanCodeRelease.sparsification.glmBasedSparsification import compute_feature_selection_and_assignment


def compute_qsenn_feature_selection_and_assignment(model, train_loader, test_loader, log_folder, num_classes, seed, n_features, per_class=5, config=None):
    feature_sel, sparse_matrices, biases, mean, std = compute_feature_selection_and_assignment(model, train_loader,
                                                                                               test_loader,
                                                                                               log_folder, num_classes, seed, n_features, config=config)
    weight_sparse, bias_sparse = get_sparsified_weights_for_factor(
        sparse_matrices[:-1], biases[:-1], per_class)  # Last one in regularisation path has no regularisation
    print(
        f"Number of nonzeros in weight matrix: {torch.sum(weight_sparse != 0)}")
...
def get_sparsified_weights_for_factor(weights, biases, factor,):
    ...
    if goal_nonzeros < len(values):
        threshold = (values[int(goal_nonzeros) - 1] +
                     values[int(goal_nonzeros)]) / 2
    else:
        threshold = values[-1]
    max_val = torch.max(torch.abs(values))
    weight_sparse = discretize_2_bins_to_threshold(
        no_reg_result_mat, threshold, max_val)
    ...
    diff_n_weight = total_weight_count_per_class - \
        np.min(total_weight_count_per_class)
...
def discretize_2_bins_to_threshold(data, treshold, max):
    boundaries = torch.tensor(
        [-max, -treshold, treshold, max], device=data.device)
    ...
    discretized_tensor = means.cpu(
    )[bucketized_tensor.cpu() - 1].to(bucketized_tensor.device)
