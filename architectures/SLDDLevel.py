...
class SLDDLevel(torch.nn.Module):
    def __init__(self, 
        ...
        if len(selected_mean) != len(self.selection):
            selected_mean = selected_mean[self.selection]
            selected_std = selected_std[self.selection]

        self.mean = torch.nn.Parameter(selected_mean, 
                                       requires_grad=retrain_normalisation)
        self.std = torch.nn.Parameter(selected_std, 
                                      requires_grad=retrain_normalisation)

        if bias is not None:
            self.layer = torch.nn.Linear(n_features, 
                                         num_classes)
            
            self.layer.bias = torch.nn.Parameter(bias, 
                                                 requires_grad=False)
        else:
            self.layer = torch.nn.Linear(n_features, 
                                         num_classes, 
        ...
        self.layer.weight = torch.nn.Parameter(weight_at_selection, 
                                               requires_grad=False)
