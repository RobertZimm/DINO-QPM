...
from CleanCodeRelease.helpers.file_system import gen_sample_paths
...
def remove_indices(tensor: torch.Tensor,
    ...
    if dim < -num_dims or dim >= num_dims:
        raise ValueError(f"Dimension {dim} is out of bounds for tensor with {num_dims} dimensions.")
    ...
    if isinstance(indices, int):
        indices_to_remove = torch.tensor([indices], device=tensor.device, dtype=torch.long)
        ...
        if not torch.is_tensor(indices) or indices.dtype not in [torch.long, torch.int]:
            ...
            try:
                indices_to_remove = indices.to(device=tensor.device, dtype=torch.long)
            except Exception as e:
                raise TypeError(f"Input 'indices' tensor must contain integers. Error: {e}")
        else:
            indices_to_remove = indices.to(device=tensor.device, dtype=torch.long)  # Ensure device/dtype
    else:
        raise TypeError("Input 'indices' must be an int, list, tuple, or tensor.")
    ...
    if torch.any(indices_to_remove < 0) or torch.any(indices_to_remove >= size):
        invalid_indices = indices_to_remove[(indices_to_remove < 0) | (indices_to_remove >= size)]
        raise IndexError(f"Indices {invalid_indices.tolist()} are out of bounds for dimension {dim} with size {size}")
...
def compare_tensors(tensor1, tensor2, print_diffs=True):
    ...
    if tensor1.shape != tensor2.shape:
        print(f"Tensors have different shapes: {tensor1.shape} vs {tensor2.shape}")
        return False

    if tensor1.dtype != tensor2.dtype:
        print(f"Tensors have different data types: {tensor1.dtype} vs {tensor2.dtype}")
        return False
    ...
    if print_diffs:
        ...
        for idx in zip(*diff_indices):
            ...
            print(f"  Difference at index {idx}: {val1} (tensor1) vs {val2} (tensor2)")
...
def load_images(num_classes: int = 4,
    ...
    if class_idx is not None:
        print(">>> Loading images for class", class_idx, "with", samples_per_class, "samples per class.")
...
def prep_img(img_path: str | Path,
    ...
    if apply_mask:
        ...
        if mask is None:
            mask_full_path = img_path.replace("images", "segmentations").replace(".jpg", ".png")
...
def normalize_attention_heads(
    ...
    if attn_map.dim() == 3:
        ...
        min_vals = torch.amin(attn_map, dim=(1, 2), keepdim=True)  # Shape: (Heads, 1, 1)
        max_vals = torch.amax(attn_map, dim=(1, 2), keepdim=True)  # Shape: (Heads, 1, 1)
    elif attn_map.dim() == 2:
        # Input shape is (Heads, Num_Patches)
        # Calculate min/max over the patches dimension (1)
        min_vals = torch.amin(attn_map, dim=1, keepdim=True)  # Shape: (Heads, 1)
        max_vals = torch.amax(attn_map, dim=1, keepdim=True)  # Shape: (Heads, 1)
    else:
...
def apply_heatmap_on_arrays(image_array,
    ...
    if not isinstance(image_array, np.ndarray) or not isinstance(mask_array, np.ndarray):
        raise TypeError("Inputs 'image_array' and 'mask_array' must be NumPy arrays.")

    if image_array.ndim not in [2, 3]:
        raise ValueError("Input 'image_array' must have 2 (Grayscale) or 3 (RGB/RGBA) dimensions.")
    if mask_array.ndim != 2:
        raise ValueError("Input 'mask_array' must be 2-dimensional (grayscale).")
    ...
    try:
        if image_array.ndim == 3:
            if image_array.shape[2] == 4:  # RGBA
                ...
                base_image = Image.fromarray(image_array, 'RGB').convert('RGBA')  # Convert to RGBA for blending
            else:
                raise ValueError(f"Unsupported image array channel depth: {image_array.shape[2]}")
        ...
        raise ValueError(f"Could not convert input NumPy arrays to PIL Images: {e}") from e
    ...
    if (img_h, img_w) != (mask_h, mask_w):
        print(f"Warning: Image shape {(img_h, img_w)} and mask shape {(mask_h, mask_w)} differ.")
        print(f"Resizing mask to match image size: {(img_h, img_w)}")
        # Resize using PIL. Note: resize takes (width, height)
        mask_image_pil = mask_image_pil.resize((img_w, img_h), Image.Resampling.LANCZOS)
    ...
    try:
        ...
        print(f"Error: Colormap '{colormap_name}' not found. Using 'jet' instead.")
    ...
    colored_mask_uint8[:, :, 3] = alpha_channel  # Set the alpha channel (index 3)
