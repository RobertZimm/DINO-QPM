import torch
import numpy as np
from CleanCodeRelease.architectures.qpm_dino.similarity_functions import (
...
def get_repr_prot_loss(feat_embeddings: torch.Tensor, prototypes: torch.Tensor,
    ...
    if prototypes.dim() == 3:
        ...
        prototypes = prototypes.view(-1, d)  # (num_prototypes*num_sub_prototypes, D)
        multi_dim = True
    else:
        multi_dim = False
    ...
    if multi_dim:
        similarity = similarity.view(-1, n_prototypes, n_sub_prots)  # (B * num_patches, num_prototypes, num_sub_prototypes)
