...
import torch
import yaml
from CleanCodeRelease.data.data_loaders import DinoData
...
from CleanCodeRelease.helpers.img_tensor_arrays import apply_heatmap_on_arrays, interpolate_patches
from CleanCodeRelease.model.dino_model import Dino2Div
from CleanCodeRelease.model.load_model import load_final_model, load_qpm_feature_selection_and_assignment
...
def visualise_maps(class_idx: int,
    ...
    if not transpose:
        fig, axes = plt.subplots(num_img_per_column, len(x_tensors),
                                 figsize=(2 * len(x_tensors), 2 * num_img_per_column))  # Adjust figure size as needed
