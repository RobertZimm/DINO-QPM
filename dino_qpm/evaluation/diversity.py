...
class MultiKCrossChannelMaxPooledSum:
    ...
    def get_max_ks(self, weights):
        # TODO compute the number of different values per class
        if isinstance(weights, torch.Tensor):
            nonzeros = torch.count_nonzero(weights, 1)
        else:
            nonzeros = torch.count_nonzero(torch.from_numpy(weights), 1)
        return nonzeros
    ...
    def adapt_feature_maps(self, outputs, initial_feature_maps):
        ...
        if relevant_weights.shape[1] != feature_maps.shape[1]:
            # TODO compute weighted sum of feature maps, like avg of 1 * 2 + 1* avg(2)
