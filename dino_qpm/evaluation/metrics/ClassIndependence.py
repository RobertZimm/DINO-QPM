...
def compute_real_gt_max(features,
                        weights,
                        labels):
    no_min_features = features - features.min(dim=0)[0]
    no_min_denominator = no_min_features.sum(dim=0)
    no_min_features_part = torch.zeros((weights.shape[0],
                                        features.shape[1],))

    for c in labels.unique():
        gt_mask = labels == c
        upper_nominator = no_min_features[gt_mask].sum(dim=0)
        no_min_features_part[c] = upper_nominator / torch.clamp(no_min_denominator,
                                                                min=1e-8)

    count_non_zero = no_min_features_part.count_nonzero(dim=0)
