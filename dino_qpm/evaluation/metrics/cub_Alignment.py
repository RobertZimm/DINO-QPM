...
def get_cub_alignment_from_features(features_train_sorted):
    metric_matrix = compute_metric_matrix(np.array(features_train_sorted.cpu().numpy()),
                                          "train")
...
def compute_metric_matrix(features, mode):
    ...
    matrix_shape = (features.shape[1],
                    max(image_attribute_labels["attribute"]) + 1)
    ...
    for attribute_id, group in grouped_attributes:
        ...
        is_present_avg = np.mean(features[is_present["img_id"]],
                                 axis=0)
        not_present_avg = np.mean(features[not_present["img_id"]],
                                  axis=0)
    ...
    no_abs_feature_mean = metric_matrix / np.clip(no_abs_features.mean(axis=0)[:, None],
                                                  a_min=1e-10,
                                                  a_max=None)
