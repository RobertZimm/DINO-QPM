...
def compute_locality_bias(train_loader: torch.utils.data.DataLoader,
                          model: torch.nn.Module):
    loader = torch.utils.data.DataLoader(train_loader.dataset,
                                         batch_size=64,
                                         shuffle=False,
                                         num_workers=4)
    features = []
    ...
    locality_bias = compute_average_softmax(features,
                                            loader,
                                            model,
                                            device)
    mask_bias = compute_edge_mask(loader,
                                  model,
                                  device,
                                  features)
    ...
    feature_bias = (normed_loc_bias * torch.tensor(normed_mask_bias >= 0).float().numpy()
                    + normed_loc_bias.min()
                    * torch.tensor(normed_mask_bias < 0).float().numpy())
...
def get_assignments(features: list):
...
def get_range_of_biggest_cluster(features_in: torch.Tensor):
...
def remove_outliers_and_scale(feature_bias: torch.Tensor,
                              scale: float = np.sqrt(10) * 0.1):
    feature_bias = torch.tensor(feature_bias)
    high_end, low_end = get_range_of_biggest_cluster(feature_bias)

    # Clip all outliers from biggest clusters to extreme values
    feature_bias[feature_bias < low_end] = (torch.abs(low_end)
                                            * torch.sign(feature_bias[feature_bias < low_end]))

    feature_bias[feature_bias > high_end] = (torch.abs(high_end)
                                             * torch.sign(feature_bias[feature_bias > high_end]))
...
def compute_average_softmax(features_train: torch.Tensor,
    ...
    weights = (features_train / features_train.sum(
        dim=1, keepdim=True)).to(device)
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(loader):
            ...
            answer += torch.sum(max_map *
                                weights[i * input.shape[0]: (i + 1) * input.shape[0]], dim=0)
...
def check_on_edge(maps: torch.Tensor):
    argmaxes = torch.argmax(maps.flatten(-2), dim=(2))
    x = torch.div(argmaxes,
                  maps.shape[2],
                  rounding_mode='trunc')

    y = torch.remainder(argmaxes, maps.shape[2])
...
def compute_edge_mask(loader: torch.utils.data.DataLoader,
                      model: torch.nn.Module,
                      device: torch.device,
    ...
    with torch.no_grad():
        for i, (input, target) in tqdm(enumerate(loader), total=len(loader)):
            ...
            answer += torch.sum(diff_to_middle *
                                weights[i * 64:(i + 1) * 64], dim=0)

    return -answer.cpu().numpy()


def center_bias(loader: torch.utils.data.DataLoader,
                features: torch.Tensor,
                model: torch.nn.Module):
    device = torch.device(
        'cuda') if torch.cuda.is_available() else torch.device('cpu')
...
def diff_to_edge(maps: torch.Tensor):
    argmaxes = torch.argmax(maps.flatten(-2), dim=(2))
    x = torch.div(argmaxes,
                  maps.shape[2],
                  rounding_mode='trunc')
