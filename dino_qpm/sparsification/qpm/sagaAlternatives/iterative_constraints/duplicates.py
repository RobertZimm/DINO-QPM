    ...
    def would_line_be_duplicate(self, questionable_class_index, questionable_feature_index):
        ...
        for second_idx in relevant_classes:
            if (questionable_line == self.selected_edge_tensor[:, second_idx]).all():
                print("CheckDuplicateLine would be duplicate")
                return True
        return False
...
class SwapDuplicateChecker:
    ...
    def would_line_be_duplicate(self, questionable_class_index, questionable_feature_index, remove_from_class_index):
        ...
        for second_idx in relevant_classes:
            if (questionable_line == self.selected_edge_tensor[:, second_idx]).all():
                print("CheckSwapDuplicateLine would be duplicate")
                return True
        return False
    ...
    while len(remining_duplicates) > 0:
        ...
        for sngl_class in [c1, c2]:
            ...
            for remove_feature in assigned_features:
                ...
                while would_be_duplicate and len(assignable_features) > 0:
...
def add_uniqueness_constraint(m, duplicates, edges, selected_features, previous_constraints, features, total_relevant,
                              prev_dubs, edges_restricted, existing_edges, existing_edge_tensor,
                              new=True, mask_selected=True):
    diffs = []
    us = []
    # if new:
    # Calculating full Duplicates, i.e. all classes that share all features
    full_duplicates = {(c1, c2): prev_dubs[(c1, c2)] for c1, c2 in prev_dubs}
    # feature_counter = DuplicateCounter()
    for c1, c2 in duplicates:
        equal_entries1 = np.nonzero(existing_edge_tensor[:, c1])
        equal_entries2 = np.nonzero(existing_edge_tensor[:, c2])
        equal_entries = np.union1d(equal_entries1, equal_entries2)
        if (c1, c2) in prev_dubs:
            equal_entries = np.union1d(equal_entries, prev_dubs[(c1, c2)])
        full_duplicates[(c1, c2)] = equal_entries
    #     feature_counter.add_features(equal_entries)
    # feature_counter.calculate_feasiblity()
    # for c1,c2 in prev_dubs:
    #     if not (c1,c2) in full_duplicates:
    #         full_duplicates[(c1,c2)] = prev_dubs[(c1,c2)]
    ...
    for class_idx, second_class in duplicates:
        ...
        if new:
            if mask_selected:
                ...
                diff = m.addMVar(total_relevant.shape[0], lb=-1, vtype=GRB.INTEGER, name="diff")
            u = m.addVar(name="u", vtype=GRB.INTEGER)
            if edges_restricted:
                if mask_selected:
                    constraints.append(m.addConstr(diff == existing_edges[feature_indices, class_idx] - existing_edges[
                    ...
                    constraints.append(
                        m.addConstr(
                            diff == (existing_edges[total_relevant, class_idx] - existing_edges[
                                total_relevant, second_class]), f"Aux_{class_idx}_{second_class}"))
            else:
                constraints.append(
                    m.addConstr(
                        diff == (edges[total_relevant, class_idx] - edges[total_relevant, second_class]) * features[
                            total_relevant], f"Aux_{class_idx}_{second_class}"))
            constraints.append(m.addConstr(u == gp.norm(diff, 1), f"Norm_Deduplication_{class_idx}_{second_class}"))
            constraints.append(m.addConstr(u >= 1, f"Deduplication_{class_idx}_{second_class}"))
            us.append(u)
        else:
            diff = m.addVar(lb=-GRB.INFINITY, vtype=GRB.CONTINUOUS, name="diff")
            if edges_restricted:
                constraints.append(
                    m.addConstr(
                        diff == (edges[total_relevant, class_idx] * edges[total_relevant, second_class]).sum(), "Aux"))
            else:
                constraints.append(
                    m.addConstr(
                        diff == ((edges[total_relevant, class_idx] * edges[total_relevant, second_class]) * features[
                            total_relevant]).sum(), "Aux"))
            constraints.append(m.addConstr(
                diff <= gp.max_(edges[total_relevant, class_idx].sum(), edges[total_relevant, second_class].sum()),
                "Deduplication"))
